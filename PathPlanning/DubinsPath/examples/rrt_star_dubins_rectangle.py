...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        width= np.random.uniform(1.0 ,5.0)     ## 적절한 폭과 길이로 설정하시면 됩니다.
        height= np.random.uniform(1.0 ,5.0)
        yaw = np.random.uniform(0,2 * np.pi)

        obstacle = RectangleObstacle(x, y, width, height, yaw)
    ...
    for i in range(200):
        ...
        if rrt_star.is_collision_free(nearest_node_state, new_node_state, obstacles):
            near_node_ids = rrt_star.get_near_node_ids(new_node_state, draw=False)
